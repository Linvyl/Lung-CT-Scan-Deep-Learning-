...
for i in range(mask.shape[0]):
    ...
    if len(contours) > 0:
        ...
        cv2.drawContours(convex_hull_mask, [convex_hull], 0, (255, 255, 255), cv2.FILLED)

# Apply Dilation to the mask
dilated_mask = cv2.dilate(convex_hull_mask, kernel, iterations=1)

# find contours of the mask to plot only the lungs
mask_contours, _ = cv2.findContours(dilated_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
